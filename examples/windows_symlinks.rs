...
fn main() -> anyhow::Result<()> {
    ...
    match windows_symlinks::create_symlink(&link1, &file1) {
        Ok(_) => println!("✓ Created file symlink: {} -> {}", link1.display(), file1.display()),
    ...
    match windows_symlinks::create_symlink(&link2, &subdir) {
        Ok(_) => println!("✓ Created directory symlink: {} -> {}", link2.display(), subdir.display()),
    ...
    match windows_symlinks::create_symlink(&link3, Path::new("file2.txt")) {
        Ok(_) => println!("✓ Created relative symlink: {} -> file2.txt", link3.display()),
