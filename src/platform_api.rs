...
impl PlatformCopier {
    ...
    fn copy_file_windows(&self, source: &Path, dest: &Path) -> Result<u64> {
        ...
        use std::ffi::OsStr;
        use winapi::um::winbase::{CopyFileExW, COPY_FILE_FAIL_IF_EXISTS};
        ...
        unsafe extern "system" fn progress_callback(
            ...
            stream_number: DWORD,
            callback_reason: DWORD,
            source_file: *const u16,
            destination_file: *const u16,
            data: LPVOID,
        ) -> DWORD {
            if !data.is_null() {
                let callback_data = &*(data as *const CallbackData);
                if let Some(ref progress) = callback_data.progress {
                    let bytes = *total_bytes_transferred.QuadPart() as u64;
                    let total = *total_file_size.QuadPart() as u64;
                    if total > 0 {
    ...
    #[cfg(target_os = "linux")]
    fn try_copy_file_range(&self, src: &std::fs::File, dst: &mut std::fs::File, total_size: u64) -> Result<u64> {
    ...
    #[cfg(target_os = "linux")]
    fn copy_with_sendfile(&self, src: &std::fs::File, dst: &mut std::fs::File, total_size: u64) -> Result<u64> {
        use std::os::unix::io::AsRawFd;
...
mod tests {
    ...
    fn test_platform_copy() {
        ...
        let bytes = copier.copy_file(&source, &dest).unwrap();
